...
　　路上，他经过了一个小镇，发现小镇的人们都围在一棵树下争吵。
  ...
  即，大家希望能选出一个人，所有人都认识他，但同时他不认识镇上除自己以外的其他人（在此，我们默认每个人自己认识自己）。可是小镇里的人太多了，一下子大家谁也说服不了谁。
...
/*
 * 首先，其他人都认识镇长，镇长不认识任何人
 * 说明镇长只能有一个
 * 记录某人被认识的人数（跳过自己认识自己的记录）
 * 若为总人数减一，且他不认识任何人，则符合镇长的要求
 */
...
int main(){
    ...
    while(T--){
        ...
        vector< record > vec;
        for(int i=0; i<n; i++)
            vec.push_back(record(false));
        int A, B;
        for(int i=0; i<m; i++){
            cin>>A>>B;
            if(A==B)
                continue;
            vec[A-1].knowelse = true;
            vec[B-1].s.insert(A-1);
        }
        bool isexict = false;
        for(int i=0; i<n; i++){
            if(vec[i].s.size() == n-1 && vec[i].knowelse == false){
                isexict = true;
                cout<<"1"<<endl;
                cout<<i+1<<endl;
                break;
